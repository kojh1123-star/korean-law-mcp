...
export function installAdmin(app: Express, options: {
  ...
  router.get("/", (req, res) => {
    ...
    const serviceCards = '<h2>서비스 연결 및 처리 상태</h2><p><small>키 설정 여부는 실제 인증 성공과 다릅니다. HTTP 처리 오류는 인증 후 MCP 요청의 HTTP 오류이며, 정상 응답에 포함된 외부 API 오류는 별도로 도구 응답을 확인하세요. 서버 재시작 후에도 집계를 유지합니다.</small></p><div class="cards">' + SERVICE_IDS.map(id => { const m = store.serviceMetrics(id); const configured = !!process.env[id === "law" ? "LAW_OC" : id === "g2b" ? "G2B_API_KEY" : "KOSIS_TOKEN"]; return `<div class="card"><h3>${SERVICES[id].name}</h3><p>인증키: ${configured ? "설정됨" : "미설정"}</p><p>HTTP 요청 ${m.requests}회 · 오류 ${m.errors}회<br>최근 정상 HTTP 응답: ${stamp(m.last_success)}<br>최근 HTTP 오류: ${stamp(m.last_error)}</p><p>연결 주소<br><code style="overflow-wrap:anywhere">${esc(issuer + SERVICES[id].path)}</code></p></div>` }).join("") + '</div>'
