...
import { TRAFFIC_STAGE_LABELS, type TrafficEvent } from "./traffic.js"
...
export class OAuthStore {
  ...
  constructor(path: string, issuer: string) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS recent_traffic(id INTEGER PRIMARY KEY, at INTEGER NOT NULL, category TEXT, method TEXT, status INTEGER, auth TEXT, account_id TEXT, calls INTEGER NOT NULL, stage TEXT NOT NULL DEFAULT 'legacy');
    `)
    ...
    if (!this.db.prepare("PRAGMA table_info(employee_usage)").all().some(row => row.name === "service")) this.db.exec(`
      ...
      COMMIT;
    `)
    if (!this.db.prepare("PRAGMA table_info(recent_traffic)").all().some(row => row.name === "stage")) {
      this.db.exec("ALTER TABLE recent_traffic ADD COLUMN stage TEXT NOT NULL DEFAULT 'legacy'")
    }
  ...
  recordTraffic(event: TrafficEvent, now = Date.now()) {
    ...
    const stage = event.stage && Object.prototype.hasOwnProperty.call(TRAFFIC_STAGE_LABELS, event.stage) ? event.stage : "other"
    ...
    if (["law", "g2b", "kosis", "oauth"].includes(event.group)) {
      this.db.prepare("INSERT INTO recent_traffic(at,category,method,status,auth,account_id,calls,stage) VALUES(?,?,?,?,?,?,?,?)")
        .run(at, event.group, method, event.status, event.auth, event.accountId || null, calls, stage)
  ...
  trafficSummary(period: "today" | "month" | "all", now = Date.now()) {
    ...
    return { startedAt: Number(this.setting("traffic-started-at", () => String(Math.floor(now / 1000)))), rows,
      recent: this.db.prepare("SELECT at,category,method,status,auth,account_id,calls,stage FROM recent_traffic ORDER BY id DESC LIMIT 30").all() }
  }
